...
API_TOKEN = '199832151:AAFbxdtetMvO0Q9BbgzKu2FKb6AaUmRbYEU'
		...
		if m.new_chat_participant.id == bot.get_me().id:
			...
			if str(cid) not in chat:
				...
				with open('chat.txt', 'a') as f:
					f.write(str(cid)+"\n")
			bot.send_message(cid, “salam mamnun ke mano invite kardid in note support baraye teame SoRbLaCk hast Ì†ΩÌ∏â\nbaraye didane aazaye team /credits ra type konid Ì†ΩÌ∏á")
			...
			groupid = m.chat.id
			bot.send_message('-117568338', "# DEBUG # " + "Bot got invited to the group " + str(groupname) + "(" + str(groupid) + ")", parse_mode="HTML")
...
def credits(m):
	cid = m.chat.id
	bot.send_message(cid, “@RyanGmorr \n@SorBlack \n@mohammad20162015 ”, parse_mode="Markdown")

@bot.message_handler(commands=['help'])
def help(m):
	cid = m.chat.id
	bot.send_message(cid, "Baraye Ferestadane Peygham be ma az /ask <peyghametun> estefade konid”, parse_mode="Markdown")
...
def ask(m):
    ...
    str = m.text
    txt = str.replace("/ask", "")
    bot.send_message(idA, "<b>Mamnun az nazare shoma \n nazare shoma:</b> {} \n<b>Chat-Id :</b> {} \n<b>say mikonim zud javabe shomaro bedim</b>".format(txt,idA), parse_mode="HTML")
    bot.send_message('-117568338', "<b>#NEW MSG\n Chat-Id:</b> {} \n<b>Msg :</b> {}".format(idA,txt), parse_mode="HTML")
    bot.send_message(idA, "<b>Soale Shoma Deliver shod</b>", parse_mode="HTML")
...
def answer(m):
        ...
        bot.send_message(to_id, "<b>.</b> <i>{}</i> <b>javabe soale shoma:</b>\n <i>{}</i>".format(from_id,text), parse_mode="HTML")
				...
				with open('user.txt', 'a') as f:
					f.write(str(id)+"\n")
		                print "New user received."
	                        bot.send_message(id, “Baraye dadane bazar az /ask <peyghametun> estefade konid”, parse_mode="Markdown")
        return
...
def check(m):
        ...
        for line in datafile:
            if str(id) not in datafile:
                bot.send_message(cid, “Faghat baraye admin ha”, parse_mode="Markdown")
                return
